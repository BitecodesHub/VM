...
export const TEMPLATES = {
  ...
  'linux-desktop': {
    ...
    // 'audio' publishes KasmVNC's audio-out websocket (container :4901, streams
    // MPEG-TS) so the panel can proxy it at /m/<name>/kasmaudio for the bundled
    // jsmpeg speaker player. Decoding is client-side, so the server only relays.
    // 'audio' = speaker (container :4901); 'mic' = microphone (container :4903,
    // KasmVNC's tornado audio-input WS). Both proxied under /m/<name>/ so a bundled
    // client can stream them; both stay loopback-published.
    ports: [{ role: 'ui', containerPort: 6901, range: [6201, 6299] }, { role: 'audio', containerPort: 4901, range: [4911, 4999] }, { role: 'mic', containerPort: 4903, range: [4811, 4899] }],
    // password mode 'none': Kasm auth is entirely the injected Basic auth.
    ui: { path: '/vnc.html?autoconnect=true&resize=remote&reconnect=true&reconnect_delay=2000', password: { mode: 'none' } },
  },
  'icewm-desktop': {
    id: 'icewm-desktop',
  ...
  'chrome-node': {
    ...
    memory: '2048m',
    cpus: 2,
    // 16:9 framebuffer so resize=scale fills a normal browser window (noVNC on
    // seleniarm's x11vnc cannot live-resize like the KasmVNC desktops, so we make
    // the fixed screen widescreen instead of the stock ~4:3 that letterboxed).
    env: [{ name: 'SE_SCREEN_WIDTH', value: '1920' }, { name: 'SE_SCREEN_HEIGHT', value: '1080' }, { name: 'SE_SCREEN_DEPTH', value: '24' }],
    ports: [
...
export function mapContainerToCard(inspect) {
  ...
  const card = {
    ...
    audioPort: null,       // KasmVNC audio-out host port; proxied at /m/<name>/kasmaudio
    micPort: null,         // KasmVNC audio-in host port; proxied at /m/<name>/kasmmic
  ...
  if (managed) {
    ...
    card.uiPort = uiPort;
    // Audio-out host port: prefer the label buildRunArgs records, else derive
    // from the published container :4901 (media desktops only).
    if (t?.ports?.some((p) => p.role === 'audio')) {
      card.audioPort = labels['vmpanel.audio.port']
        ? parseInt(labels['vmpanel.audio.port'], 10)
        : (hostPortFor(inspect, 4901) || null);
    }
    if (t?.ports?.some((p) => p.role === 'mic')) {
      card.micPort = labels['vmpanel.mic.port']
        ? parseInt(labels['vmpanel.mic.port'], 10)
        : (hostPortFor(inspect, 4903) || null);
    }
    if (uiPort) {
  ...
  if (adopted) {
    ...
    if (adopted.media) { card.audioPort = hostPortFor(inspect, 4901) || null; card.micPort = hostPortFor(inspect, 4903) || null; }
...
export function buildRunArgs({ template, name, ports, createdAt, owner, webdriverBind = LOOPBACK, cap = false, hostWebcam = false }) {
  ...
  // Audio-out: publish KasmVNC's audio websocket (container :4901). The panel
  // proxies it at /m/<name>/kasmaudio; a bundled jsmpeg player decodes it, so
  // desktop speaker audio works without the paid Kasm Workspaces parent.
  const audio = t.ports.find((p) => p.role === 'audio');
  if (audio && ports.audio) {
    args.push('-l', `vmpanel.audio.port=${ports.audio}`);
    args.push('-p', `${LOOPBACK}:${ports.audio}:${audio.containerPort}`);
  }
  // Microphone: publish KasmVNC's audio-input WS (container :4903). Proxied at
  // /m/<name>/kasmmic; the bundled prism-mic.js streams s16le PCM to it, which
  // the server turns into a `virtual_mic` PulseAudio source.
  const mic = t.ports.find((p) => p.role === 'mic');
  if (mic && ports.mic) {
    args.push('-l', `vmpanel.mic.port=${ports.mic}`);
    args.push('-p', `${LOOPBACK}:${ports.mic}:${mic.containerPort}`);
  }
