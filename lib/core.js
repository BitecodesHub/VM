...
export const TEMPLATES = {
  // Both desktops are KasmVNC-based so speaker, microphone AND camera work by
  // DEFAULT (VNC desktops carry only pixels+input — they cannot do audio). Each
  // serves its web client + every media channel over ONE HTTPS port with Basic
  // auth; the panel proxies it under /m/<name>/ and satisfies the auth
  // server-side, so the embedded screen never prompts. XFCE is kasm's built-in
  // DE; IceWM is launched via the image's custom_startup.sh (START_XFCE4=0).
  'linux-desktop': {
    id: 'linux-desktop',
    label: 'Linux Desktop — XFCE',
    description: 'Full Ubuntu XFCE desktop — speaker, microphone and camera all work',
    hint: 'Recommended · mic + camera',
  ...
  'icewm-desktop': {
    ...
    description: 'Snappier lightweight desktop — also with speaker, microphone and camera',
    hint: 'Lightweight · mic + camera',
