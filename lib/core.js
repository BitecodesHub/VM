...
export const PANEL_TEMPLATES = new Set(['linux-desktop', 'icewm-desktop', 'media-desktop', 'chrome-node', 'firefox-node']);
...
export const TEMPLATES = {
  'linux-desktop': {
    id: 'linux-desktop',
    label: 'Linux Desktop — Modern (XFCE)',
    description: 'Full Ubuntu desktop (XFCE) in your browser',
    hint: 'Recommended',
    image: 'minimal-linux-desktop:xfce',
    namePrefix: 'desktop',
    uploadDir: '/home/linuxuser/Desktop',
    shmSize: '512m',
    memory: '1536m',
    cpus: 2,
    ports: [{ role: 'ui', containerPort: 6080, range: [6081, 6199] }],
    ui: { path: '/vnc.html?autoconnect=true&resize=scale&reconnect=true&reconnect_delay=2000', password: { mode: 'query', value: 'secret' } },
  },
  'icewm-desktop': {
    id: 'icewm-desktop',
    label: 'Linux Desktop — Lightweight (IceWM)',
    description: 'Smaller, snappier IceWM desktop for lighter use',
    hint: 'Lightweight',
  ...
  'media-desktop': {
    id: 'media-desktop',
    label: 'Linux Desktop — Media (audio + camera)',
    description: 'XFCE desktop with working speaker, microphone and webcam',
    hint: 'Mic · Speaker · Camera',
    image: 'minimal-media-desktop:xfce',
    namePrefix: 'media',
    uploadDir: '/home/kasm-user/Uploads',
    shmSize: '512m',
    memory: '2048m',
    cpus: 2,
    // Media flags: `media` drives UI badges/hints; `needsWebcam` maps a host
    // v4l2loopback device into the container WHEN the host provides one (see
    // buildRunArgs + the server's hostWebcam probe). Audio (speaker+mic) needs
    // no device and always works.
    media: true,
    needsWebcam: true,
    // The kasmweb image reads its Basic-auth / session password from VNC_PW.
    env: [{ name: 'VNC_PW', value: 'secret' }],
    // KasmVNC serves the web client AND every media channel (screen, audio-out,
    // mic, webcam, uploads) over ONE HTTPS port, all authenticated with Basic
    // auth. The panel proxies it under /m/<name>/ (the client uses relative
    // asset paths and honours the noVNC `path` setting, so its WebSocket rides
    // /m/<name>/websockify) and satisfies the Basic auth SERVER-SIDE, so the
    ...
    // password mode 'none': Kasm auth is entirely the injected Basic auth; there
    // is no separate RFB password to pass in the query string.
...
export function listTemplates() {
  return Object.values(TEMPLATES).map((t) => ({ id: t.id, label: t.label, image: t.image, description: t.description, hint: t.hint || null }));
}
...
function adoptByImage(image) {
  if (/(^|\/)minimal-media-desktop\b/.test(image) || /^minimal-media-desktop:/.test(image)) {
    return { template: 'media-desktop', ...TEMPLATES['media-desktop'] };
  }
  if (/(^|\/)minimal-linux-desktop\b/.test(image) || /^minimal-linux-desktop:/.test(image)) {
    return { template: 'linux-desktop', ...TEMPLATES['linux-desktop'] };
  }
